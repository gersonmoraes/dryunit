...
let _ = (module Internal_runner : Extension_api.Runner)
include Internal_runner
...
module Theme = struct
  ...
  module Default : S = struct
    module Colors= struct
      ...
      let bold   s = sprintf "\x1B[1m%s\x1B[0m" s
      (* let header_color v = cyan v *)
      let header_color v = gray v
    end
...
module Terminal = struct
  ...
  let header ?(sep='-') ~header_color ~bold name =
    let cols = cols () in
    let separator n =
...
let print_loc suite test =
  let line, start, len =
    Location.parse_loc test.test_loc
  in
...
let run ?(colors=true) ?(name="Main") suites =
  ...
  List.iter
    ( fun suite ->
      ...
      Terminal.header ~header_color ~bold @@ sprintf "%s with %d tests" suite.suite_name (List.length suite.tests);
      eprintf "\n";
      let failed = ref [] in
      List.iter
        ( fun (test: test) ->
    ...
    suites;
  Terminal.header ~header_color ~bold "Status";
  eprintf "\n";
